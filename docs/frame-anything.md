# Frame Anything: RAPP Workspace/1 Grail manager

**RAPP-valid != accurately observed != semantically faithful != currently
authorized != safely deployable.**

This is a local, candidate-protocol integration, not a native migration,
deployment service, learned-capability claim, or signed activation. Existing
`rapp-workspace-manager/1` routing remains backward compatible. The Grail
sidecar never replaces the manager RAPPID, registry, source worlds, or tasks.
Its protocol ID is uniquely `rapp-workspace/grail-1.0`; RAPP/1 is unchanged.
...
`tools/grail_runtime.py`, not selected by a received manifest. `bind` verifies
...
captured validator schemas/provenance. The only candidate evaluator is the
...
[`examples/grail/scopes.synthetic.json`](../examples/grail/scopes.synthetic.json):
...
```bash
python3 -B tools/workspace_manager.py grail init \
  --workspace "<EXISTING_PRIVATE_MANAGER>" \
...
this candidate refuses it pending a separately reviewed policy transition.
...
```bash
python3 -B tools/workspace_manager.py grail capture \
...
```bash
python3 -B tools/workspace_manager.py grail run \
  --workspace "<EXISTING_PRIVATE_MANAGER>" --job document \
...
```bash
python3 -B tools/workspace_manager.py grail stage \
  --workspace "<EXISTING_PRIVATE_MANAGER>" --job document \
...
```bash
python3 -B tools/workspace_manager.py grail adopt \
...
```bash
python3 -B tools/workspace_manager.py grail status --workspace "<EXISTING_PRIVATE_MANAGER>"
python3 -B tools/workspace_manager.py grail inspect --workspace "<EXISTING_PRIVATE_MANAGER>" --job document
python3 -B tools/workspace_manager.py grail tree --workspace "<EXISTING_PRIVATE_MANAGER>"
python3 -B tools/workspace_manager.py grail project \
  --workspace "<EXISTING_PRIVATE_MANAGER>" --allow-materialization --allow-retention
python3 -B tools/workspace_manager.py grail focus \
  --workspace "<EXISTING_PRIVATE_MANAGER>" --job document \
  --allow-materialization --allow-retention
python3 -B tools/workspace_manager.py grail recover \
  --workspace "<EXISTING_PRIVATE_MANAGER>" --allow-materialization --allow-retention
python3 -B tools/workspace_manager.py grail history \
...
**no native/source roots**. Outputs are fixed `.grail/view.json`,
`.grail/view.md`, and `.grail/estate.code-workspace`. Existing registry/editor
...
```bash
python3 -B tools/workspace_manager.py grail migrate \
...
Use the explicit candidate, never a convenient historical validator:

```bash
export GRAIL_PROTOCOL_CHECKOUT="<EXPLICIT_GRAIL_CANDIDATE_CHECKOUT>"
export RAPP1_PATH="<EXPLICIT_PINNED_RAPP1_CHECKOUT>"
python3 -B -m unittest discover -s tests -p 'test_grail_manager.py' -v
python3 -B -m unittest discover -s tests -v
python3 -m py_compile tools/*.py tests/*.py
python3 -B tools/check_conformance.py . --rapp1-path "$RAPP1_PATH"
python3 -B tools/check_grail_conformance.py \
  --protocol-checkout "$GRAIL_PROTOCOL_CHECKOUT" --rapp1-path "$RAPP1_PATH"
```

The integration tests do not discover dependencies; without both explicit
environment paths that class is skipped, **not** Grail acceptance. An
...
and the public index into a fresh owned validation mirror, runs the candidate's
unaltered safe conformance there, requires nonzero frames, and compares
candidate bytes before/after. It never writes into the supplied candidate.
